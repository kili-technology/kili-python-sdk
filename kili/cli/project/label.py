...
def import_labels(
    ...
    if len(files) > 0:
        label_paths = get_label_file_paths_to_upload(
            files, verbose)
        if len(label_paths) == 0:
            raise ValueError(
                "No label's files to upload. "
    ...
    label_index_to_import = [i for i, external_id in enumerate(
        external_ids) if external_id in assets]

    for i, external_id in enumerate(external_ids):
        if i not in label_index_to_import:
            warnings.warn(
                f'{external_id} is not an asset of project ID: {project_id}.')

    if is_prediction:
        create_predictions_arguments = generate_create_predictions_arguments(
            label_paths[label_index_to_import],
            external_ids[label_index_to_import],
