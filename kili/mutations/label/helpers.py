...
def get_label_file_paths_to_upload(files: Tuple[str, ...],
    ...
    file_paths_to_upload = [
        path for path in file_paths if path.endswith(".json")]
    if len(file_paths_to_upload) == 0:
        raise ValueError(
            "No files to upload. "
            "Check that the paths exist and that the file is .json")
    if verbose:
        ...
        if len(file_paths_to_upload) != len(file_paths):
            print('Paths skipped either do not exist '
                  'or point towards a non -json file')
