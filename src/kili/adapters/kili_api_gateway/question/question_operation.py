...
class QuestionOperationMixin(BaseOperationMixin):
    ...
    def create_questions(
        ...
        """Create questions."""
        created_questions: List[Dict[Literal["id"], QuestionId]] = []
        ...
        with tqdm.tqdm(total=total, desc="Creating questions", disable=disable_tqdm) as pbar:
            asset_id_array, asset_external_id_array, text_array = zip(
                *((q.asset_id, q.external_id, q.text) for q in questions)
            )

            real_asset_id_array = self._get_asset_ids_or_throw_error(
                asset_id_array, asset_external_id_array, project_id  # type: ignore
            )
            for batch_questions in BatchIteratorBuilder(list(zip(real_asset_id_array, text_array))):
                variables = {
                    "issues": [
                        {"issueNumber": 0, "type": "QUESTION", "assetId": asset_id, "text": text}
                        for (asset_id, text) in batch_questions
                    ],
                    "where": {"idIn": real_asset_id_array},
                }
