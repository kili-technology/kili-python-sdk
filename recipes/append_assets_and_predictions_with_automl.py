...
def plot_most_frequent_words_or_ngrams(figure_path, X_train, ngram_range=(1, 1)):
    ...
    all_ngrams, all_counts = zip(
        *[(n, c) for c, n in sorted(zip(all_counts, all_ngrams), reverse=True)])
...
def print_infos_dataset(X_train, y_train, X_test, y_test):
    print('Train samples shape: {} | train labels shape: {}'.format(
        X_train.shape, y_train.shape))
    print('Test samples shape: {} | test labels shape: {}'.format(
    ...
    for _class in uniq_class_arr:
        print('Counts for class ',
              uniq_class_arr[_class], ': ', counts[_class])
    ...
    for asset in assets:
        for label in asset["labels"]:
            if label["labelType"] == "DEFAULT":
                ...
                y_train.append(
                    converter_name_to_label[jsonResponse["categories"][0]["name"]])
...
def automl_train_and_predict(X_train, y_train, X_to_be_predicted, y_to_be_predicted):
    x_train, x_to_be_predicted = ngram_vectorize(
        X_train, y_train, X_to_be_predicted)
    ...
    print("Precision|Recall|F1Score|Support: {}".format(
        precision_recall_fscore_support(y_to_be_predicted, predictions)))
...
def main(api_endpoint):
    ...
    X_train_origin, y_train_origin, X_test, y_test = load_dataset_and_predictions(
        data_path, plot_path)
    ...
    while STOP_CONDITION:
        ...
        if len(X_train) > 5:
            ...
            predictions = automl_train_and_predict(
                X_train, y_train, X_test, y_test)
            # Insert pre-annotations
            for external_id, prediction in enumerate(tqdm(predictions)):
                json_response = {"categories": [
                    {"name": converter_label_to_name[prediction], "confidence":100}]}
                playground.create_prediction(
                    asset_id=converter_external_id_to_id[external_id], json_response=json_response)
            print("Done.\n")
