...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='data/ultralytics49.pt', help='initial weights')
    ...
    PASSWORD = 'abc123'
    API_ENDPOINT = 'https://staging.cloud.kili-technology.com/api/label/graphql'
    PROJECT_ID = 'ck3p1hdlihmv50781gzs9gfjb'
    ...
    def convert_from_yolo_to_google_format(label):
        if 'jsonResponse' not in label:
            return []
        print('LABEL', label)
        print('JSON', label['jsonResponse'])
        import json
    ...
    for _X, _y in zip(X, y):
        ...
        with open(os.path.join(images_folder, f'{filename}.jpg'), 'wb') as f:
            print(f'Downloading {_X}...')
            response = requests.get(_X, stream=True)
            if not response.ok:
                continue
            for block in response.iter_content(1024):
                if not block:
                    break
                f.write(block)
        annotations = convert_from_yolo_to_google_format(_y)
        with open(os.path.join(labels_folder, f'{filename}.txt'), 'wb') as f:
            for category, x, y, w, h in annotations:
                print('THIS IS A CATGORY', category)
                f.write(f'{category} {x} {y} {w} {h}\n'.encode())  # class is hard coded!! LIKE IN COCO.NAMES

    with open('data/coco_1cls.data', 'wb') as f:
        for filename in filenames:
            f.write(f'{os.path.join(images_folder, filename)}.jpg\n'.encode())
