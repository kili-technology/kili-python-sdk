...
class TestAssetsNamespaceIntegration:
    ...
    def test_nested_namespaces_available(self, mock_kili_client):
        ...
        assets_ns = mock_kili_client.assets_ns
    ...
    def test_domain_api_method_delegation(self, mock_kili_client):
        """Test that domain API methods properly delegate to legacy methods."""
        # Mock the gateway get_project method to return proper project data
        mock_kili_client.kili_api_gateway.get_project.return_value = {"inputType": "IMAGE"}

        # Mock the legacy methods
        mock_kili_client.append_many_to_dataset = MagicMock(
            return_value={"id": "project_123", "asset_ids": ["asset1"]}
        )
        mock_kili_client.delete_many_from_dataset = MagicMock(return_value={"id": "project_123"})
        mock_kili_client.update_properties_in_assets = MagicMock(return_value=[{"id": "asset1"}])

        assets_ns = mock_kili_client.assets_ns

        # Test create delegation
        result = assets_ns.create(
            project_id="project_123", content_array=["https://example.com/image.png"]
        )
        assert result["id"] == "project_123"
        mock_kili_client.append_many_to_dataset.assert_called_once()

        # Test delete delegation
        result = assets_ns.delete(asset_ids=["asset1"])
        assert result["id"] == "project_123"
        mock_kili_client.delete_many_from_dataset.assert_called_once()

        # Test update delegation
        result = assets_ns.update(asset_ids=["asset1"], priorities=[1])
        assert result[0]["id"] == "asset1"
        mock_kili_client.update_properties_in_assets.assert_called_once()
    ...
    def test_workflow_operations_delegation(self, mock_kili_client):
        ...
        # Mock the legacy workflow methods
        mock_kili_client.assign_assets_to_labelers = MagicMock(return_value=[{"id": "asset1"}])
        mock_kili_client.send_back_to_queue = MagicMock(
            return_value={"id": "project_123", "asset_ids": ["asset1"]}
        )
        mock_kili_client.add_to_review = MagicMock(
            return_value={"id": "project_123", "asset_ids": ["asset1"]}
        )
        ...
        assert result[0]["id"] == "asset1"
        mock_kili_client.assign_assets_to_labelers.assert_called_once()

        # Test workflow step invalidate
        result = assets_ns.workflow.step.invalidate(asset_ids=["asset1"])
        assert result["id"] == "project_123"
        mock_kili_client.send_back_to_queue.assert_called_once()

        # Test workflow step next
        result = assets_ns.workflow.step.next(asset_ids=["asset1"])
        assert result["id"] == "project_123"
        mock_kili_client.add_to_review.assert_called_once()
    ...
    def test_metadata_operations_delegation(self, mock_kili_client):
        ...
        # Mock the legacy metadata methods
        mock_kili_client.add_metadata = MagicMock(return_value=[{"id": "asset1"}])
        mock_kili_client.set_metadata = MagicMock(return_value=[{"id": "asset1"}])
        ...
        assert result[0]["id"] == "asset1"
        mock_kili_client.add_metadata.assert_called_once()
        ...
        assert result[0]["id"] == "asset1"
        mock_kili_client.set_metadata.assert_called_once()
    ...
    def test_external_ids_operations_delegation(self, mock_kili_client):
        ...
        # Mock the legacy external IDs method
        mock_kili_client.change_asset_external_ids = MagicMock(return_value=[{"id": "asset1"}])
        ...
        assert result[0]["id"] == "asset1"
        mock_kili_client.change_asset_external_ids.assert_called_once()
