...
@patch.object(ProjectQuery, "__call__", side_effect=mocked__ProjectQuery)
@patch.object(AssetQuery, "__call__", side_effect=mocked__project_assets)
def test_import(_mocker_asset, _mocker_project, _mocker_kili, test_case):
    runner = CliRunner()
    with runner.isolated_filesystem():
        ...
        with patch("kili.services.asset_import.import_assets") as mocked_import_assets_service:
            ...
            debug_subprocess_pytest(result)
            mocked_import_assets_service.assert_called_with(*test_case["expected_service_payload"])
...
@patch.object(ProjectQuery, "__call__", side_effect=mocked__ProjectQuery)
@patch.object(AssetQuery, "__call__", side_effect=mocked__project_assets)
def test_export(_mocker_asset, _mocker_project, _mocker_kili, name, test_case):
    runner = CliRunner()
    with runner.isolated_filesystem():
